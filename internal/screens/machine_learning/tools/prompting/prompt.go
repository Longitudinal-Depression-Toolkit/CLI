...
func promptParameterValue(parameter schema.Parameter) (any, error) {
	...
	if components.IsNumericParameterType(typeName) {
		return components.PromptNumericParameter(components.NumericParameterPromptConfig{
			...
			Type:        typeName,
			Hint:        parameter.Hint,
			Description: numericDescriptionForParameter(parameter),
	...
	if shouldUseCheckboxMultiSelect(parameter) {
		return promptCheckboxMultiSelectValue(parameter)
	}
	if shouldUseValidationSplitSelector(parameter) {
		return promptValidationSplitValue(parameter)
	}
...
func shouldUsePathPickerForParameter(parameter schema.Parameter) bool {
	...
	if strings.Contains(key, "notebook_output_path") || strings.Contains(key, "notebook path") {
		return false
	}
	return strings.Contains(key, "path") ||
		strings.Contains(key, "folder") ||
		strings.Contains(key, "directory")
}
...
func shouldUseCheckboxMultiSelect(parameter schema.Parameter) bool {
	key := strings.ToLower(strings.TrimSpace(parameter.Key))
	return (key == "metric_keys" || key == "excluded_estimators") && len(parameter.Options) > 0
}

func promptCheckboxMultiSelectValue(parameter schema.Parameter) (any, error) {
	key := strings.ToLower(strings.TrimSpace(parameter.Key))
	defaults := parseDefaultCSV(parameter.Default)
	defaultLookup := make(map[string]struct{}, len(defaults))
	for _, value := range defaults {
		defaultLookup[value] = struct{}{}
	}

	options := make([]components.MultiSelectOption, 0, len(parameter.Options))
	for _, option := range parameter.Options {
		value := strings.TrimSpace(fmt.Sprintf("%v", option.Value))
		if value == "" {
			continue
		...
		_, selected := defaultLookup[value]
		if key == "excluded_estimators" && len(defaultLookup) == 0 {
			selected = true
		}
		options = append(options, components.MultiSelectOption{
			Label:       label,
			Value:       value,
	...
	description := "Space toggles, Enter confirms, Esc exits."
	if key == "excluded_estimators" {
		description = "Select estimators to include. Ticked estimators will run; unticked ones will be ignored."
	}

	selected, err := components.PromptMultiSelect(
		parameterPromptTitle(parameter),
		description,
		options,
	)
	if err != nil {
	...
	if key == "excluded_estimators" {
		if len(selected) == 0 {
			return nil, fmt.Errorf("%s requires at least one estimator to be included", parameterPromptName(parameter))
		}
		includedLookup := make(map[string]struct{}, len(selected))
		for _, include := range selected {
			includedLookup[include] = struct{}{}
		}
		excluded := make([]string, 0, len(options))
		for _, option := range options {
			if _, included := includedLookup[option.Value]; included {
				continue
			}
			excluded = append(excluded, option.Value)
		}
		if len(excluded) == 0 {
			return "none", nil
		}
		sort.Strings(excluded)
		return strings.Join(excluded, ","), nil
	}

	if len(selected) == 0 {
		if parameter.Required {
			return nil, fmt.Errorf("%s requires at least one metric", parameterPromptName(parameter))
		}
		return nil, nil
	}
...
func parseDefaultCSV(value any) []string {
	raw := strings.TrimSpace(defaultString(value))
	if raw == "" || strings.EqualFold(raw, "none") {
		return nil
	}
	return model.ParseCSVValues(raw)
}

func numericDescriptionForParameter(parameter schema.Parameter) string {
	key := strings.ToLower(strings.TrimSpace(parameter.Key))
	switch key {
	case "cv_folds":
		return "Choose the number of folds. Example: 5 means train on 4 folds and validate on 1 fold in rotation."
	case "random_seed":
		return "Set the reproducibility seed. Use the same value to reproduce dataset splits and model randomness."
	default:
		return strings.TrimSpace(parameter.Hint)
	}
}

func shouldUseValidationSplitSelector(parameter schema.Parameter) bool {
	return strings.EqualFold(strings.TrimSpace(parameter.Key), "validation_split")
}

func promptValidationSplitValue(parameter schema.Parameter) (any, error) {
	selected := defaultString(parameter.Default)
	options := []huh.Option[string]{
		huh.NewOption("auto", "auto"),
		huh.NewOption("90/10", "90/10"),
		huh.NewOption("80/20", "80/20"),
		huh.NewOption("70/30", "70/30"),
		huh.NewOption("60/40", "60/40"),
		huh.NewOption("50/50", "50/50"),
		huh.NewOption("40/60", "40/60"),
	}
	if strings.TrimSpace(selected) == "" {
		selected = "auto"
	}
	...
	form := components.NewLDTForm(
		huh.NewGroup(
			huh.NewSelect[string]().
				Title(parameterPromptTitle(parameter)).
				Description("Choose train/validation split. Example: 80/20 means 80% train, 20% validation.").
				Options(options...).
				Value(&selected),
		),
	)
