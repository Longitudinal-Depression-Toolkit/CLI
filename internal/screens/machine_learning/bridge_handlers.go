...
func HandleBridgePath(path []string) (bool, error) {
	...
	if helpRequested {
		return true, renderMachineLearningHelp(canonical)
	}

	if tool, ok := ToolFromPath(canonical); ok {
		listOnly := strings.TrimSpace(tool.ListCommand) != "" && ContainsToken(canonical, tool.ListCommand)
		return true, runMachineLearningToolFlow(tool, listOnly)
	}
	if len(canonical) >= 3 && canonical[1] == "presets" {
		return true, runMachineLearningPresetFlowByID(canonical[2])
	}

	switch {
	case len(canonical) == 1:
		return true, runMachineLearningHub()
	case len(canonical) == 2 && canonical[1] == "tools":
		return true, runMachineLearningToolsHub()
	case len(canonical) == 2 && canonical[1] == "presets":
		return true, runMachineLearningPresetsHub()
	default:
		return true, fmt.Errorf(
			"unsupported machine learning route in Go CLI: %s",
			strings.Join(path, " "),
		)
	}
}

func renderMachineLearningHelp(canonical []string) error {
	if tool, ok := ToolFromPath(canonical); ok {
		return runMachineLearningToolFlow(tool, true)
	}
	if len(canonical) >= 3 && canonical[1] == "presets" {
		return runMachineLearningPresetFlowByID(canonical[2])
	}
	...
	if len(canonical) >= 2 && canonical[1] == "tools" {
		...
		components.PrintLine(theme.App.SubtitleStyle().Render("Machine learning tools"))
	...
	if len(canonical) >= 2 && canonical[1] == "presets" {
		...
		components.PrintBlankLine()
		components.PrintLine(theme.App.SubtitleStyle().Render("Machine learning presets"))
	...
	components.PrintBlankLine()
	components.PrintLine(theme.App.SubtitleStyle().Render("Machine learning"))
...
func LoadNode(path []string) (*parsedHelp, bool, error) {
	...
	case len(canonical) == 1:
		return newNode(
			cfg.RootSummary,
			[]commandDef{
				{Name: "tools", DisplayName: "Tools", Description: "Primary machine learning tools for new studies."},
	...
	return nil, true, fmt.Errorf("unknown machine learning path: %s", strings.Join(path, " "))
}
