...
func loadConfig() (Config, error) {
	...
	if len(merged.Tools) == 0 {
		return Config{}, errors.New("data preprocessing config produced zero tools; keep at least one tool")
	}
	return merged, nil
}

func readBundledConfig() (Config, error) {
	raw, embedErr := toolkitconfig.ReadBundledConfigFile(defaultDataPreprocessingConfigPath)
	if embedErr == nil {
		return decodeConfig(raw, defaultDataPreprocessingConfigPath)
	}

	basePath, pathErr := findBundledConfigPath()
	if pathErr != nil {
		return Config{}, fmt.Errorf(
			"failed to load bundled data preprocessing config, embedded error: %v, filesystem error: %w",
			embedErr,
			pathErr,
		)
	}
...
func findBundledConfigPath() (string, error) {
	candidates := []string{
		defaultDataPreprocessingConfigPath,
		filepath.Join("cli", defaultDataPreprocessingConfigPath),
		filepath.Join("src", "cli", defaultDataPreprocessingConfigPath),
	}

	if root, err := common.FindProjectRoot(); err == nil {
		candidates = append(
			candidates,
			filepath.Join(root, "src", "cli", defaultDataPreprocessingConfigPath),
			filepath.Join(root, "cli", defaultDataPreprocessingConfigPath),
		)
	}
	...
	return "", fmt.Errorf("data preprocessing config file not found; expected %s", defaultDataPreprocessingConfigPath)
}

func findOverrideConfigPath() (string, bool, error) {
	if value := strings.TrimSpace(os.Getenv(dataPreprocessingConfigEnvVar)); value != "" {
...
func readConfigFile(path string) (Config, error) {
	...
	if err != nil {
		return Config{}, fmt.Errorf("failed to read data preprocessing config at %s: %w", path, err)
	}
