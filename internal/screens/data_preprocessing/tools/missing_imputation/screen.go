...
func printSummary(selected schema.Technique, result map[string]any) {
	...
	components.PrintLine(theme.App.SubtitleStyle().Render("Missing imputation complete"))
	...
	components.PrintfLine("- Missing after: %v", result["missing_after"])
}
