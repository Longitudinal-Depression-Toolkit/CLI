package combinedatasetwithtrajectories
...
func printSummary(selected schema.Technique, result map[string]any) {
	...
	components.PrintLine(theme.App.SubtitleStyle().Render("Dataset combination complete"))
	...
	components.PrintfLine("- Merge type: %v", result["merge_type"])
