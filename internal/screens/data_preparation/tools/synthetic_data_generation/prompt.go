...
func promptParameterValue(parameter schema.Parameter) (any, error) {
	...
	if typeName == "path" && shouldUsePathPickerForParameter(parameter) {
		...
		confirmedPath, err := promptPathValueInline(parameter, selectedPath)
		if err != nil {
			return nil, err
		}
		return parseInputParameter(parameter, confirmedPath)
	}
...
func promptPathValueInline(parameter schema.Parameter, initialPath string) (string, error) {
	value := strings.TrimSpace(initialPath)
	title := parameterPromptName(parameter)

	inputField := huh.NewInput().
		Title(title).
		Description("Selected path. Press Enter to continue, edit if needed. Esc to exit tool.").
		Value(&value).
		Validate(func(raw string) error {
			trimmed := strings.TrimSpace(raw)
			if trimmed == "" && parameter.Required {
				return fmt.Errorf("%s is required", parameterPromptName(parameter))
			}
			return nil
		})

	placeholder := strings.TrimSpace(parameter.Placeholder)
	if placeholder == "" {
